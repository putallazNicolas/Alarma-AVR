...
char *passwords[] = {"1234", "5678", "0812"}; // En realidad habria que implementar un sistema de hasheo...
...
void setupPassword()
{
  for (int i = 1; i <= USERS; i++)
  {
    ...
    {
      passwords[i][j] = eeprom_read_byte((uint8_t*)(j + i * 4));
    }
  }
}
...
void configurePasswords()
{
  ...
  for (int i = EEPROM_index; i < EEPROM_index + 4; i++)
  {
    eeprom_write_byte((uint8_t*)i, input[i]); 
  }
...
void checkValidPasswords()
{
  for (int i = 0; i < USERS; i++)
  {
    uint8_t isValid = strCompare(passwords[i], "\0\0\0\0", PASSWORD_LENGTH); 
    if (!isValid)
    {
      passwords[i] = "1234";
    }
